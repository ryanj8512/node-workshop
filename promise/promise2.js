...
job1
    .then ((result) => {
        ...
        return doWork("打開電腦", 2000, true)
    })
    .then ((result) => {
        console.log("看錄影檔", result);
        return doWork("看錄影檔", 2000, true)
    })
    .then ((result) => {
        console.log("帶小孩散步", result);
        return doWork("帶小孩散步", 8000, true)
    })
    .then ((result) => {
        console.log("繼續看書", result);
        return doWork("繼續看書", 5000, true)
    })
    .then ((result) => {
        console.log("睡覺", result);
        return doWork("睡覺", 8000, true)
    })
    .catch ((reject) => {
        console.log("任務失敗", reject);
    })
    .finally (() => {
        console.log("今日事今日畢!");
    })
